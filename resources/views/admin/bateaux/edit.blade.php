...
@section('title', 'Modifier un bateau - Administration')
        ...
        <main class="lg:col-span-3">
            <div class="mb-6">
                <div class="flex items-center justify-between">
                    <div>
                        <h1 class="text-3xl font-bold text-gray-800">Modifier un bateau</h1>
                        <p class="text-gray-600 mt-2">{{ $bateau->modele }}</p>
                    </div>
            ...
            <form id="editBateauForm" action="{{ route('admin.bateaux.update', $bateau) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="bg-white rounded-xl shadow-md p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="modele" value="{{ old('modele', $bateau->modele) }}" required
                        ...
                        <div>
                            ...
                            <select name="type_id" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                                ...
                                @foreach($types as $type)
                                    <option value="{{ $type->id }}" {{ old('type_id', $bateau->type_id) == $type->id ? 'selected' : '' }}>
                                        {{ $type->libelle }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="zone_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                                ...
                                @foreach($zones as $zone)
                                    <option value="{{ $zone->id }}" {{ old('zone_id', $bateau->zone_id) == $zone->id ? 'selected' : '' }}>
                                        {{ $zone->libelle }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="number" name="prix" value="{{ old('prix', $bateau->prix) }}" required min="0" step="0.01"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>
                        ...
                        <div>
                            ...
                            <select name="slogan_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                                ...
                                @foreach($actions as $action)
                                    <option value="{{ $action->id }}" {{ old('slogan_id', $bateau->slogan_id) == $action->id ? 'selected' : '' }}>
                                        {{ $action->libelle }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="md:col-span-2">
                            <div class="flex items-center space-x-6">
                                ...
                                <label class="flex items-center">
                                    <input type="checkbox" name="occasion" value="1" {{ old('occasion', $bateau->occasion) ? 'checked' : '' }}
                                        class="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="description" rows="4" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ old('description', $bateau->description) }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-xl shadow-md p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <input type="text" name="chantier" value="{{ old('chantier', $bateau->chantier) }}"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Architecte</label>
                            <input type="text" name="architecte" value="{{ old('architecte', $bateau->architecte) }}"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Année</label>
                            <input type="number" name="annee" value="{{ old('annee', $bateau->annee) }}" min="1900" max="{{ date('Y') + 1 }}"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Pavillon</label>
                            <input type="text" name="pavillon" value="{{ old('pavillon', $bateau->pavillon) }}"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Matériaux</label>
                            <input type="text" name="materiaux" value="{{ old('materiaux', $bateau->materiaux) }}"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Longueur HT (m)</label>
                            <input type="number" name="longueurht" value="{{ old('longueurht', $bateau->longueurht) }}" step="0.01"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Largeur (m)</label>
                            <input type="number" name="largeur" value="{{ old('largeur', $bateau->largeur) }}" step="0.01"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Tirant d'eau (m)</label>
                            <input type="number" name="tirantdeau" value="{{ old('tirantdeau', $bateau->tirantdeau) }}" step="0.01"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Poids lège (kg)</label>
                            <input type="number" name="poidslegeencharges" value="{{ old('poidslegeencharges', $bateau->poidslegeencharges) }}" step="0.01"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Surface au près (m²)</label>
                            <input type="number" name="surfaceaupres" value="{{ old('surfaceaupres', $bateau->surfaceaupres) }}" step="0.01"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Moteur</label>
                            <input type="text" name="moteur" value="{{ old('moteur', $bateau->moteur) }}"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Puissance (CV)</label>
                            <input type="number" name="puissance" value="{{ old('puissance', $bateau->puissance) }}"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Nombre de moteurs</label>
                            <input type="number" name="nombre_moteurs" value="{{ old('nombre_moteurs', $bateau->nombre_moteurs) }}" min="0"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Heures moteur</label>
                            <input type="number" name="heuresmoteur" value="{{ old('heuresmoteur', $bateau->heuresmoteur) }}"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Système antidérive</label>
                            <input type="text" name="systemeantiderive" value="{{ old('systemeantiderive', $bateau->systemeantiderive) }}"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Cabines</label>
                            <input type="number" name="cabines" value="{{ old('cabines', $bateau->cabines) }}"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Passagers</label>
                            <input type="number" name="passagers" value="{{ old('passagers', $bateau->passagers) }}"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>

                        <div class="md:col-span-3">
                            <label class="block text-sm font-medium text-gray-700 mb-2">Mots-clés (SEO)</label>
                            <input type="text" name="mots" value="{{ old('mots', $bateau->mots) }}" placeholder="bateau, voilier, catamaran..."
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>
                        ...
                        <div class="md:col-span-3">
                            ...
                            <textarea name="symboles" rows="2" placeholder="GPS, pilote automatique, guindeau électrique..."
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ old('symboles', $bateau->symboles) }}</textarea>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="relative group">
                            ...
                            <div class="absolute top-2 right-2 flex gap-1">
                                ...
                                <button type="button"
                                        ...
                                        class="bg-red-500 hover:bg-red-600 text-white p-2 rounded-full opacity-0 group-hover:opacity-100 transition"
                            ...
                            <div class="absolute bottom-2 left-2">
                                <span class="bg-black bg-opacity-70 text-white text-xs px-2 py-1 rounded flex items-center gap-1">
                                    @if($media->type === 'image' && $media->ordre === 0)
                                        <i class="fas fa-star text-yellow-400"></i>
                                    @endif
                                    {{ $media->type === 'image' ? 'Photo' : 'Vidéo' }} #{{ $media->ordre }}
                                </span>
                            </div>
                        </div>
                        @endforeach
                            ...
                            <div class="mb-6">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                    @foreach($equipementsCat as $equipement)
                                        <label class="flex items-center cursor-pointer p-3 border rounded-lg hover:bg-blue-50 transition {{ in_array($equipement->id, $selectedEquipements) ? 'bg-blue-50 border-blue-500' : 'border-gray-300' }}">
                                            <input type="checkbox" name="equipements[]" value="{{ $equipement->id }}"
                                                   class="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                                                   {{ in_array($equipement->id, $selectedEquipements) ? 'checked' : '' }}>
                ...
                <div class="bg-white rounded-xl shadow-md p-6">
                    ...
                    <div class="space-y-6">
                        <!-- Upload Photos -->
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Nouvelles photos</label>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">
                                <i class="fab fa-youtube text-red-600 mr-2"></i>Nouvelles vidéos YouTube
                            </label>
...
window.setMainMedia = function(mediaId) {
    ...
    form.method = 'POST';
    form.action = `/admin/media/${mediaId}/set-main`;
...
document.addEventListener('DOMContentLoaded', function() {
    const form = document.getElementById('editBateauForm');
    ...
    if (submitBtn) {
        // Ajouter un listener sur la soumission du formulaire
        ...
        console.error('❌ ERREUR: Bouton submit NON trouvé !');
    }
});
